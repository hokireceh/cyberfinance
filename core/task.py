...
def claim_daily(token, proxies=None):
...
def process_check_in(token, proxies=None):
    claim_daily_info = claim_daily(token=token, proxies=proxies)
    try:
        day = claim_daily_info["day"]
        reward = claim_daily_info["reward"]
        base.log(
            f"{base.white}Auto Check-in: {base.green}Success {base.white}| {base.green}Day: {base.white}{day} - {base.green}Reward: {base.white}{reward:,}"
        )
    except:
        base.log(f"{base.white}Auto Check-in: {base.red}{claim_daily_info}")
